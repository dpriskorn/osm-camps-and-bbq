...
OUTPUT_GPX = f"output/firepits_sweden_{date.today().strftime('%Y%m%d')}.gpx"


def filter_firepits():
    tmpdir = tempfile.mkdtemp()
    try:
        firepits_pbf = os.path.join(tmpdir, "firepits.pbf")
        firepits_geojson = os.path.join(tmpdir, "firepits.geojson")

        print("Filtering firepits...")
        subprocess.run(
            [
                "osmium", "tags-filter",
                PBF_PATH,
        ...
        gdf = gpd.read_file(firepits_geojson)
        print(f"  Total firepits: {len(gdf)}")
...
def write_gpx(gdf):
    lines = [
        ...
        '<gpx version="1.1" creator="firepits-exporter"',
        '     xmlns="http://www.topografix.com/GPX/1/1"',
        '     xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"',
        '     xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">',
        '  <metadata>',
        '    <name>Firepits in Sweden</name>',
        '  </metadata>',
    ]
    ...
    for _, row in gdf.iterrows():
        ...
        if pd.isna(name) or not name or str(name) == "nan":
            name = f"{osm_type}_{osm_id}" if osm_id else "Firepit"
    ...
    print(f"Wrote {len(gdf)} firepits to {OUTPUT_GPX}")

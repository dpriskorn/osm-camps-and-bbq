...
OUTPUT_GPX = f"output/public_baths_sweden_{date.today().strftime('%Y%m%d')}.gpx"


def filter_public_baths():
    tmpdir = tempfile.mkdtemp()
    try:
        bathing_pbf = os.path.join(tmpdir, "bathing.pbf")
        bathing_geojson = os.path.join(tmpdir, "bathing.geojson")

        print("Filtering public baths...")
        subprocess.run(
            [
                "osmium", "tags-filter",
                PBF_PATH,
        ...
        print(f"  Total public baths: {len(gdf)}")
...
def write_gpx(gdf):
    lines = [
        ...
        '<gpx version="1.1" creator="public-bath-exporter"',
        '     xmlns="http://www.topografix.com/GPX/1/1"',
        '     xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"',
        '     xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">',
        '  <metadata>',
        '    <name>Public Baths in Sweden</name>',
        '  </metadata>',
    ]
    ...
    for _, row in gdf.iterrows():
        ...
        if pd.isna(name) or not name or str(name) == "nan":
            name = f"{osm_type}_{osm_id}" if osm_id else "Public Bath"
    ...
    print(f"Wrote {len(gdf)} public baths to {OUTPUT_GPX}")

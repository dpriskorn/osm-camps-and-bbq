...
OUTPUT_GPX = f"output/bathing_places_sweden_{date.today().strftime('%Y%m%d')}.gpx"


def filter_bathing_places():
    tmpdir = tempfile.mkdtemp()
    try:
        bathing_pbf = os.path.join(tmpdir, "bathing.pbf")
        bathing_geojson = os.path.join(tmpdir, "bathing.geojson")

        print("Filtering bathing places...")
        subprocess.run(
            [
                "osmium", "tags-filter",
                PBF_PATH,
        ...
        gdf = gpd.read_file(bathing_geojson)
        print(f"  Total bathing places: {len(gdf)}")
...
def write_gpx(gdf):
    lines = [
        ...
        '<gpx version="1.1" creator="bathing-place-exporter"',
        '     xmlns="http://www.topografix.com/GPX/1/1"',
        '     xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"',
        '     xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">',
        '  <metadata>',
        '    <name>Bathing Places in Sweden</name>',
        '  </metadata>',
    ]
    ...
    for _, row in gdf.iterrows():
        ...
        if pd.isna(name) or not name or str(name) == "nan":
            name = f"{osm_type}_{osm_id}" if osm_id else "Bathing Place"
    ...
    print(f"Wrote {len(gdf)} bathing places to {OUTPUT_GPX}")


if __name__ == "__main__":
    filter_bathing_places()

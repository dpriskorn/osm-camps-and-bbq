...
OUTPUT_GPX = f"output/lean_tos_sweden_{date.today().strftime('%Y%m%d')}.gpx"


def filter_lean_tos():
    tmpdir = tempfile.mkdtemp()
    try:
        lean_tos_pbf = os.path.join(tmpdir, "lean_tos.pbf")
        lean_tos_geojson = os.path.join(tmpdir, "lean_tos.geojson")

        print("Filtering lean tos...")
        subprocess.run(
            [
                "osmium", "tags-filter",
                PBF_PATH,
        ...
        gdf = gpd.read_file(lean_tos_geojson)
        print(f"  Total lean tos: {len(gdf)}")

        if "shelter_type" in gdf.columns:
            gdf = gdf[gdf["shelter_type"] == "lean_to"].copy()
            print(f"  After shelter_type=lean_to: {len(gdf)}")
...
def write_gpx(gdf):
    lines = [
        ...
        '<gpx version="1.1" creator="lean-tos-exporter"',
        '     xmlns="http://www.topografix.com/GPX/1/1"',
        '     xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"',
        '     xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">',
        '  <metadata>',
        '    <name>Lean tos in Sweden</name>',
        '  </metadata>',
    ]
    ...
    for _, row in gdf.iterrows():
        ...
        if pd.isna(name) or not name or str(name) == "nan":
            name = f"{osm_type}_{osm_id}" if osm_id else "Lean To"
    ...
    print(f"Wrote {len(gdf)} lean tos to {OUTPUT_GPX}")
